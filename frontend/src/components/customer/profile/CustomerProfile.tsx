...
import { openDocument, useDocumentUrl, type DocumentSource } from "@/hooks/useDocumentUrl";
...
/**
 * How each onboarding state reads to the customer. The colours are tuned for
 * the navy header the badge sits on, where the light card variants would
 * disappear.
 */
...
const DOC_LABEL: Record<CustomerDocumentType, string> = {
...
function DocumentRow({ doc, source }: { doc: CustomerDocument; source: DocumentSource }) {
    ...
    <div className="flex items-center justify-between gap-4 py-3">
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-muted-foreground">
          {doc.fileName} - {formatBytes(doc.sizeInBytes)} - uploaded {prettyDate(doc.createdAt)}
...
interface CustomerProfileProps {
  ...
  /**
   * Someone other than the customer is looking (an admin reviewing the record),
   * so the self-service controls and the submission confirmation are left off.
   */
  readOnly?: boolean;
  /** Signed blob storage link for one of this customer's documents. */
...
export function CustomerProfile({
  ...
  const justSubmitted =
    !readOnly && Boolean((location.state as { justSubmitted?: boolean } | null)?.justSubmitted);
  ...
  // them: a customer reads their own, an admin reads them through the admin API.
  ...
  const personName = [data.firstName, data.lastName].filter(Boolean).join(" ").trim();
  const name = data.companyName || personName || data.email;
  ...
  return (
    <>
          ...
          <div className="flex min-w-0 items-center gap-5">
            <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-3xl bg-white/10 text-xl font-semibold ring-1 ring-white/20 sm:h-24 sm:w-24">
              {logoUrl ? (
                ...
                <span aria-hidden>{initialsOf({ displayName: name, email: data.email })}</span>
              )}
            </div>
            <div className="min-w-0">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 2xl:grid-cols-3">
        <InfoCard icon={Building2} title="Customer Information">
          ...
          <Row label="Customer ID">{value(data.cid)}</Row>
          <Row label="Creation date">{prettyDate(data.creationDate)}</Row>
        </InfoCard>
        ...
        <InfoCard icon={Receipt} title="Billing">
          <Row label="Term">{value(data.billing?.term)}</Row>
          <Row label="Billing type">
            {value(
              data.billing?.billingType
        ...
        <InfoCard
          ...
          action={
            <span className="text-sm font-medium text-muted-foreground">{documents.length}</span>
          }
        >
          {documents.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">
