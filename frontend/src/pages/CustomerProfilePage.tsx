...
/**
 * The signed in customer's own profile: everything they have given us so far.
 *
 * The URL carries the customer's id. The API only ever answers for whoever is
 * signed in, so an id belonging to somebody else would show this customer's own
...
export function CustomerProfilePage() {
  ...
  if (!customerId || !UUID_RE.test(customerId)) return <NotFoundPage />;
      ...
      {loading ? (
        ...
        <div className="grid min-h-[60vh] w-full place-items-center">
          <div className="flex max-w-md flex-col items-center gap-4 text-center">
            ...
            <div>
              <p className="text-base font-semibold text-foreground">No customer signed in</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Sign in to a customer account to see its profile.
              </p>
            </div>
